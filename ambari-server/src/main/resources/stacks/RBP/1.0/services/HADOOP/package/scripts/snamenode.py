...
class SecondaryNameNode(HDFS):
  ...
  def stop(self, env):
    import params
    Execute('kill `cat ' + params.hadoop_pid_dir + '/secondarynamenode.pid`', user=params.hdfs_user)

  def status(self, env):
    import status_params as params
    env.set_params(params)
    pid_file = format("{hadoop_pid_dir}/secondarynamenode.pid")
    check_process_status(pid_file)

if __name__ == "__main__":
  SecondaryNameNode().execute()